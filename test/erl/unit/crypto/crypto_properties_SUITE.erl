...
-export([
    prop_sha256_size/1,
    prop_sha256_determinism/1,
    prop_sha512_size/1,
    prop_hmac_sha256_size/1,
    prop_hmac_sha256_key_sensitivity/1,
    prop_ed25519_roundtrip/1,
    prop_ed25519_wrong_key/1,
    prop_ed25519_tampered_message/1,
    prop_aes_gcm_roundtrip/1,
    prop_aes_gcm_tag_tamper/1,
    prop_aes_gcm_ct_tamper/1,
    prop_aes_gcm_wrong_aad/1,
    prop_aes_gcm_wrong_key/1,
    prop_curve25519_keypair_unique/1
]).
...
prop_sha256_size(_) ->
    forall(fun() ->
        ...
        case byte_size(Hash) of
            32 -> ok;
            N -> {fail, {input_size, byte_size(Data), output_size, N}}
        end
    end).
...
prop_sha256_determinism(_) ->
    forall(fun() ->
        ...
        case H1 =:= H2 of
            true -> ok;
            false -> {fail, {nondeterministic, Data, H1, H2}}
        end
    end).
...
prop_hmac_sha256_key_sensitivity(_) ->
    forall(fun() ->
        ...
        case M1 =/= M2 of
            true -> ok;
            false -> {fail, {hmac_key_collision, K1, K2, M1}}
        end
    end).
...
prop_ed25519_roundtrip(_) ->
    forall(fun() ->
        ...
        case signal_nif:verify_signature(Pub, Msg, Sig) of
            ok -> ok;
            Other -> {fail, {verify_failed_on_valid_sig, Other, Msg, Sig}}
        end
    end).
...
prop_ed25519_wrong_key(_) ->
    forall(fun() ->
        ...
        case signal_nif:verify_signature(Pub2, Msg, Sig) of
            ...
            {error, _} -> ok;
            Other -> {fail, {wrong_key_accepted, Other, Msg}}
        end
    end).
...
prop_ed25519_tampered_message(_) ->
    forall(fun() ->
        ...
        case signal_nif:verify_signature(Pub, Tampered, Sig) of
            invalid_signature -> ok;
            {error, _} -> ok;
            Other -> {fail, {tampered_msg_accepted, Other, Msg, Tampered}}
        end
    end).
...
prop_aes_gcm_roundtrip(_) ->
    forall(fun() ->
        ...
        case signal_nif:aes_gcm_decrypt(K, IV, CT, AAD, Tag, byte_size(PT)) of
            {ok, PT} -> ok;
            {ok, Other} -> {fail, {plaintext_mismatch, PT, Other}};
            Err -> {fail, {decrypt_failed_on_valid_ct, Err, PT}}
        end
    end).
...
prop_aes_gcm_tag_tamper(_) ->
    forall(fun() ->
        ...
        case signal_nif:aes_gcm_decrypt(K, IV, CT, AAD, BadTag, byte_size(PT)) of
            {ok, _} -> {fail, {tampered_tag_accepted, PT, Tag, BadTag}};
            _ -> ok
        end
    end).
...
prop_aes_gcm_ct_tamper(_) ->
    forall(fun() ->
        ...
        case signal_nif:aes_gcm_decrypt(K, IV, BadCT, AAD, Tag, byte_size(PT)) of
            {ok, _} -> {fail, {tampered_ct_accepted, PT, CT, BadCT}};
            _ -> ok
        end
    end).
...
prop_aes_gcm_wrong_aad(_) ->
    forall(fun() ->
        ...
        case signal_nif:aes_gcm_decrypt(K, IV, CT, AAD2, Tag, byte_size(PT)) of
            {ok, _} -> {fail, {wrong_aad_accepted, AAD1, AAD2}};
            _ -> ok
        end
    end).
...
prop_aes_gcm_wrong_key(_) ->
    forall(fun() ->
        ...
        case signal_nif:aes_gcm_decrypt(K2, IV, CT, AAD, Tag, byte_size(PT)) of
            {ok, _} -> {fail, {wrong_key_accepted, K1, K2}};
            _ -> ok
        end
    end).
