...
- **Breaking (X3DH SK 64B → 96B)**: `process_pre_key_bundle/2` and `process_pre_key_bundle_bob/5` now return a 96-byte shared secret instead of 64 bytes. The first 64 bytes are the original X3DH SK (bit-identical, since the new bytes come from extending the HKDF expansion by one more output block); the trailing 32 bytes are a shared header-key seed used by DR-HE. `init_double_ratchet/5`'s `SharedSecret` argument now requires exactly 96 bytes; passing the old 64-byte SK is rejected with `invalid_shared_secret_size`. Tuple shape of `process_pre_key_bundle/2` is unchanged (`{ok, {SK, EphemeralPub}}`), only the binary width.
- **Breaking (DR-HE wire format)**: DR messages are now in the Signal "Double Ratchet with header encryption" envelope: `version_byte(0x33) || protobuf{ enc_header=1, ciphertext=2 } || mac(8)`, where `enc_header` is `iv(16) || AES-256-CBC(header_cipher_key, iv, inner_header_protobuf)` and `inner_header_protobuf` carries the original `{ratchet_key, counter, previous_counter}` fields. `header_cipher_key` is `HKDF-SHA-256(salt=zeros, IKM=header_key, info="WhisperHeader", L=32)`. The 16-byte AES-CBC IV is generated freshly per message (libsodium `randombytes_buf`) and prepended to the ciphertext so the receiver can run constant-IV trial-decrypt across candidate header keys. The outer MAC scope is unchanged in shape (`HMAC-SHA-256(mac_key, sender_id || receiver_id || version || outer_protobuf)` truncated to 8B) but now covers the encrypted header. Pre-DR-HE wire messages cannot be decoded; the counter, previous_counter, and ratchet_key are no longer visible to an on-path observer.
- **DR state grows 128B** for the four DR-HE header-key fields (`header_key_send`, `header_key_recv`, `next_header_key_send`, `next_header_key_recv`). MKSKIPPED entries are now keyed by `(header_key, message_number)` instead of `(dh_pub, message_number)` — same total size, but the layout interpretation changes so DR state binaries from prior versions cannot interoperate.
